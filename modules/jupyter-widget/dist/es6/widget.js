...
export class DeckGLView extends DOMWidgetView {
  render() {
    ...
    const _ref = [this.model.get('width'), this.model.get('height')],
          width = _ref[0],
          height = _ref[1];
    createDeckScaffold(this.el, this.modelId, width, height);
    waitForElementToDisplay(`#deck-map-wrapper-${this.modelId}`, TICK_RATE_MILLISECONDS, this.initJSElements.bind(this));
  }

  _onViewStateChange(_ref2) {
    let viewState = _ref2.viewState;
    this.deck.setProps({
      viewState
    });
