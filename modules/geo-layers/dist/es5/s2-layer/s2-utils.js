...
function getGeoBounds(_ref2) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    for (let j = 0; j < resolution; j++) {
      ...
      const st = _s2Geometry.S2.IJToST(ij, level, offset);

      const uv = _s2Geometry.S2.STToUV(st);

      const xyz = _s2Geometry.S2.FaceUVToXYZ(face, uv);
...
function getS2Polygon(token) {
  ...
  const s2cell = _s2Geometry.S2.S2Cell.FromLatLng(_s2Geometry.S2.idToLatLng(id), level);
